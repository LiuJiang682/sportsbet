...
public class FifteenMinutesCounterTest {
	...
	public void testInitMap() {
		Map<String, Integer> map = FifteenMinutesCounter.initMap();
	...
	public void testCountAndDisplay() {
		...
		when(mockRecord3.getMinuts()).thenReturn(new Integer(58));
		when(mockRecord4.getDay()).thenReturn(WeekDay.Thursday);
		when(mockRecord4.getHour()).thenReturn(Numeral.SIX);
		when(mockRecord4.getMinuts()).thenReturn(new Integer(16));
		...
		FifteenMinutesCounter h = new FifteenMinutesCounter(this.mockList, Strings.DIRECTION_A);
		h.countAndDisplay();
	}
}
